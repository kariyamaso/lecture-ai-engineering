...
def test_feature_importance_balance(prepare_data):
    ...
    assert (
        max_importance < 0.5
    ), f"最も重要な特徴量が全体の50%以上({max_importance:.2%})を占めています。モデルが過度に1つの特徴に依存している可能性があります。"

    # 上位2つの特徴量の合計が全体の80%を超えていないことを確認
    top_two_sum = np.sum(np.sort(feature_importances)[-2:])
    assert (
        top_two_sum < 0.8
    ), f"上位2つの特徴量が全体の80%以上({top_two_sum:.2%})を占めています。モデルが少数の特徴に過度に依存している可能性があります。"

    # 全ての特徴量がある程度の重要度を持っていることを確認
    min_importance = np.min(feature_importances)
    assert (
        min_importance > 0.01
    ), f"最も重要度の低い特徴量の重要度が1%未満({min_importance:.2%})です。不要な特徴量がある可能性があります。"

    # 重要度のばらつきを確認（標準偏差が小さすぎないか）
    std_importance = np.std(feature_importances)
    assert (
        0.03 < std_importance < 0.2
    ), f"特徴量の重要度の標準偏差が適切な範囲外({std_importance:.2%})です。"
